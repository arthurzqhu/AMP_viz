...
for jvar = 1:length(vars2plot)
   ivar = vars2plot(jvar);
   for its = SBMorTAU
      nexttile((iconf-1)*16+(jvar-1)*4+1, [1,4]) % make sure it's on the right grid
      ...
      mean_score = wmean(score(iconf).(bintype{its}).(indvar_name{ivar})(:),...
         pfm.(indvar_name{ivar}).(bintype{its}).mpath_bin(:));
      ttl = sprintf('(%s) %s %sAMP-%s, weighted mean score = %.3f', ...
         Alphabet((iconf-1)*6+ivar), indvar_ename{ivar}, UorS, upper(bintype{its}), mean_score);
...
for icase = 1:length(selected_cases)
   ...
   for jvar = 1:length(vars2plot)
      ivar = vars2plot(jvar);
      nexttile((icase-1)*2+(jvar-1)*4+9,[1,2])
      time_filt_tau = bintau_struct.mean_surface_ppt*3600>sppt_th(1);
      time_filt_sbm = binsbm_struct.mean_surface_ppt*3600>sppt_th(1);

      var_comp_raw_amptau2m = amptau2m_struct.(indvar_name{ivar})(time_filt_tau,:);
      var_comp_amptau2m = var2phys(var_comp_raw_amptau2m,ivar,1);
      var_comp_raw_ampsbm2m = ampsbm2m_struct.(indvar_name{ivar})(time_filt_sbm,:);
      var_comp_ampsbm2m = var2phys(var_comp_raw_ampsbm2m,ivar,1);

      var_comp_raw_amptau4m = amptau4m_struct.(indvar_name{ivar})(time_filt_tau,:);
      var_comp_amptau4m = var2phys(var_comp_raw_amptau4m,ivar,1);
      var_comp_raw_ampsbm4m = ampsbm4m_struct.(indvar_name{ivar})(time_filt_sbm,:);
      var_comp_ampsbm4m = var2phys(var_comp_raw_ampsbm4m,ivar,1);

      if ~contains(indvar_name{ivar}, amp_only_var)
         var_comp_raw_bintau = bintau_struct.(indvar_name{ivar})(time_filt_tau,:);
         var_comp_bintau = var2phys(var_comp_raw_bintau,ivar,1);
         var_comp_raw_binsbm = binsbm_struct.(indvar_name{ivar})(time_filt_sbm,:);
         var_comp_binsbm = var2phys(var_comp_raw_binsbm,ivar,1);
      end

      hold on

      plot(time(time_filt_tau,:), var_comp_bintau,   'DisplayName', 'bin-TAU', 'Color', color_order{1}, ...
         'LineWidth', .5, 'LineStyle', '-')
      plot(time(time_filt_tau,:), var_comp_amptau2m, 'DisplayName', 'S-AMP-TAU', 'Color', color_order{1}, ...
         'LineWidth', 1.5, 'LineStyle', ':')
      plot(time(time_filt_tau,:), var_comp_amptau4m, 'DisplayName', 'U-AMP-TAU', 'Color', color_order{1}, ...
         'LineWidth', 1, 'LineStyle', '--')
      plot(time(time_filt_sbm,:), var_comp_binsbm,   'DisplayName', 'bin-SBM', 'Color', color_order{2}, ...
         'LineWidth', .5, 'LineStyle', '-')
      plot(time(time_filt_sbm,:), var_comp_ampsbm2m, 'DisplayName', 'S-AMP-SBM', 'Color', color_order{2}, ...
         'LineWidth', 1.5, 'LineStyle', ':')
      plot(time(time_filt_sbm,:), var_comp_ampsbm4m, 'DisplayName', 'U-AMP-SBM', 'Color', color_order{2}, ...
         'LineWidth', 1, 'LineStyle', '--')
      ...
      title(['(',Alphabet(icase+jvar*2),') ',var1_titl, ', ' var2_titl])
...
exportgraphics(gcf,['plots/p2/', mconfig, ' lwpspr ' bintype{SBMorTAU} ' rank sandwich.pdf'])
end % iset
