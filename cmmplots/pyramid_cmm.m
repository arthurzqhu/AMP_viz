...
%    annotation('arrow',[xl xul],[yl yu],'LineWidth',1,'color',[.5 .5 .5 .5])
%    annotation('arrow',[xr xur],[yr yu],'LineWidth',1,'color',[.5 .5 .5 .5])
% end

str1 = {'CWP: ', 'RWP: ', 'LWP: ', 'N\fontsize{8}c\fontsize{12}: ', ...
   'N\fontsize{8}r\fontsize{12}: ', 'MSP: ', 'D\fontsize{8}t\fontsize{12}: '};
str2 = {'cloud water path',...
   ...
   'raindrop number',...
   'mean surface pcpt.',...
   'D separating cloud & rain'};

annotation('rectangle',[0.655 0.771 0.18 0.15],'facecolor',tilebg_color,...
           'edgecolor',textclr)
annotation('textbox',[0.660 0.766 0.18 0.15],'String', str1,...
           'FontSize',12,'edgecolor','none','color',textclr)
annotation('textbox',[0.694 0.766 0.18 0.15],'String', str2,...
           'FontSize',12,'edgecolor','none','color',textclr)
...
title(tl,sprintf(['\\color[rgb]{%s}Bulk (AMP)\\color[rgb]{%s}-\\color[rgb]{%s}Bin',...
   '\\color[rgb]{%s} & \\color[rgb]{%s}TAU\\color[rgb]{%s}-\\color[rgb]{%s}SBM',...
   '\\color[rgb]{%s} %% difference'], num2str(hex2rgb(wrapper_color{1})), textclr_rgb, ...
   num2str(hex2rgb(wrapper_color{2})), textclr_rgb, num2str(color_order{1}), ...
   textclr_rgb, num2str(color_order{2}), textclr_rgb),'FontSize',24,'FontWeight','bold')


exportgraphics(gcf,['plots/cmmplots/pyramid_cmm.png'],'Resolution',300,'BackgroundColor',figbg_color)
