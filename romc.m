...
for its = 1:2
   bin_output_flattened = readmatrix(sprintf('rams_ml_dataset/%s_bin_%s.txt', ...
      mconfig(1:end-4), bintype{its}));
   ...
   for iconf = [1:3 5 4 6:10] 
      ...
      amp_output_flattened(:, :, iconf) = readmatrix(sprintf('rams_ml_dataset/%s_amp_%s.txt', ...
         mconfig, bintype{its}), 'ExpectedNumVariables', nvars);
   end
